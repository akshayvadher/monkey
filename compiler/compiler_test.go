...
func TestCompilerScopes(t *testing.T) {
	...
	if len(compiler.scopes[compiler.scopeIndex].instructions) != 1 {
		t.Errorf("instruction length wrong. Got=%d, want=1", len(compiler.scopes[compiler.scopeIndex].instructions))
	}
	...
	if len(compiler.scopes[compiler.scopeIndex].instructions) != 2 {
		t.Errorf("instruction length wrong. Got=%d, want=2", len(compiler.scopes[compiler.scopeIndex].instructions))
	}
...
func runCompilerTests(t *testing.T, tests []compilerTestCase) {
	...
	for _, tt := range tests {
		inputWithoutSpaces := strings.ReplaceAll(strings.ReplaceAll(tt.input, "\n", ""), "\t", "")
		t.Run(fmt.Sprintf("Test for input %q", inputWithoutSpaces), func(t *testing.T) {
